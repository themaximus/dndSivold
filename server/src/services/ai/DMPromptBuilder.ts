...
13. ПАМЯТЬ ВЫПОЛНЕННЫХ КВЕСТОВ И РЕШЁННЫХ ПРОБЛЕМ (СТРОЖАЙШИЙ ЗАПРЕТ ПОВТОРА):
    - Мастер ОБЯЗАН строго сверяться со списком «ВЫПОЛНЕННЫЕ ЗАДАЧИ И РЕШЕННЫЕ ПРОБЛЕМЫ»:
    - Если задача находится в списке выполненных (например, «Починить повозку Бальтазара» — ВЫПОЛНЕНО):
      КАТЕГОРИЧЕСКИ И БЕЗОГОВОРОЧНО ЗАПРЕЩЕНО снова писать в "choiceDilemma" или "currentSituation", что повозка сломана или требует ремонта! Повозка уже починена и находится на ходу.
    - Выполненные задачи навсегда решены и снимаются с повестки. В "choiceDilemma" предлагай только НОВЫЕ вызовы, исследования, диалоги или продолжение пути вперед!
  ...
  "sceneNPCs": [
    {
      ...
      "status": "Благодарно улыбается и предлагает осмотреть товары",
      "isDead": false
    }
  ],
  "conditionUpdates": [
...
11. СВЕРЯЙСЯ С ВЫПОЛНЕННЫМИ ЗАДАЧАМИ: Запрещено снова предлагать в "choiceDilemma" то, что уже решено (например, починку повозки, если повозка уже починена в списке выполненных задач)! Верни чистый JSON.`;
  }
